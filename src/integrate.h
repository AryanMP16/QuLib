...
double integrateOverAllKnownValues(std::vector<double> knownValues, double sep) {
    double integral = 0.0;
    int numIterations = static_cast<int>(knownValues.size());
    ...
    for (i = 0; i < numIterations; i++) {
        double weight = (i == 0 || i == numIterations - 1) ? 1 : ((i % 2 == 0) ? 2 : 4);
